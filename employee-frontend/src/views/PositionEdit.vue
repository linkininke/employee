<template>
  <div class="position-edit-container">
    <div class="page-header">
      <h1>编辑职位</h1>
      <router-link to="/positions" class="back-btn">返回列表</router-link>
    </div>
...
import { getPositionById, updatePosition } from '../api/position'
...
const handleSubmit = async () => {
  ...
  try {
    await updatePosition(positionId, form.value)
    alert('职位更新成功')
    router.push('/positions')
  } catch (error) {
